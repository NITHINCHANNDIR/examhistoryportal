...
exports.deleteStudent = async (req, res) => {
    try {
        ...
        // Delete all exam results associated with the student
...
exports.updateStudent = async (req, res) => {
    try {
        ...
        if (!student || student.role !== 'student') {
            return res.status(404).json({
                success: false,
                message: 'Student not found or not authorized to update this user type'
            });
        }

        // Check if Student ID or email is being changed and if it already exists
        ...
        // Update student profile and data
        student.email = email || student.email;
        if (studentId) student.studentId = studentId;
        
        if (firstName) student.profile.firstName = firstName;
        if (lastName) student.profile.lastName = lastName;
        if (department) student.profile.department = department;
        if (batchYear) student.profile.batchYear = parseInt(batchYear);
